...
5. Use `https://gstcranes.com/.well-known/mcp` or the Smithery listing at `https://smithery.ai/servers/gstcranes/public-discovery` for read-only MCP discovery.
...
- MCP behavior: cite `/.well-known/mcp`, `/.well-known/mcp/server-card.json`, the official MCP Registry entry, or the Smithery listing.
